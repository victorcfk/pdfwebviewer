...
let callGoogleViewerURLAppender = (url) => {
    const google_docs_url = `https://docs.google.com/viewer?url=`;
    let final_url = `${google_docs_url}${url}`;

    console.log(final_url);
    document.getElementById('constructed_url').innerHTML = final_url;
...
let regexParser = (url) => {
    ...
    if (match == null || match.length === 0) {
        ...
        document.getElementById('constructed_url').innerHTML = `Did not find a valid pdf url link in : "${url}"`;
